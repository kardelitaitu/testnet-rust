...
export async function transferLaterMemeForWallet(wallet, proxy, workerId = 1, walletIndex = 0, silent = false) {
    const startTime = Date.now();
    const actionName = '39_transferLaterMeme.js';
    ...
    try {
        ...
        if (availableTokens.length === 0) {
            ...
            return { success: false, reason: 'no_memes_minting_now' };
        }
        ...
        if (!silent) {
            console.log(`${COLORS.fg.yellow}🎲 Selected Meme: ${tokenName}${COLORS.reset}`);
