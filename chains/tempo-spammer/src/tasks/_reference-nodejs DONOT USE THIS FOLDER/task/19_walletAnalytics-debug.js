import { walletAnalyticsForWallet } from './19_walletAnalytics.js';
...
async function testWalletAnalytics() {
    console.log(`  ${COLORS.fg.magenta}🐛  DEBUG MODE: Task 19 - Wallet Analytics${COLORS.reset}\n`);
    ...
    // 2. Execute Task Once
    const result = await walletAnalyticsForWallet(wallet, proxy, 'DEBUG', 0, false);

    console.log(`\n${COLORS.fg.green}✓ Debug task completed.${COLORS.reset}`);
}

testWalletAnalytics().catch(console.error);
