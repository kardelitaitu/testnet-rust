...
async def send_token(web3, wallet, private_key, token_address, token_symbol, to_address, amount, retry_count=0):
    """Send tokens"""
    max_retries = 3
    print(f"DEBUG: send_token called with: symbol={token_symbol}, addr={token_address}, to={to_address}, amount={amount}")
    
    try:
        print(f"DEBUG: Creating contract instance for {token_address}...")
        contract = web3.eth.contract(address=Web3.to_checksum_address(token_address), abi=ERC20_ABI)
        
        print(f"DEBUG: Fetching decimals for {token_symbol}...")
