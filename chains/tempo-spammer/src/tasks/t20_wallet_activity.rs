...
impl TempoTask for WalletActivityTask {
    ...
    async fn run(&self, ctx: &TaskContext) -> Result<TaskResult> {
        ...
        let tx_count = client.provider.get_transaction_count(address).await.unwrap_or_default();

        // Get PathUSD balance
        let pathusd_addr = crate::tasks::tempo_tokens::TempoTokens::get_path_usd_address();
        let decimals = crate::tasks::tempo_tokens::TempoTokens::get_token_decimals(client, pathusd_addr)
            .await
            .unwrap_or(6);

        let balance_raw = crate::tasks::tempo_tokens::TempoTokens::get_token_balance(client, pathusd_addr, address)
            .await
            .unwrap_or(U256::ZERO);

        // Format balance with compact notation and orange color
        let balance_formatted = crate::tasks::tempo_tokens::TempoTokens::format_compact_colored(balance_raw, decimals);
