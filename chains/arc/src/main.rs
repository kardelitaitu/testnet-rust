use arc_project::config;
use arc_project::spammer;
...
async fn main() -> Result<()> {
    ...
    let config = match ArcConfig::load(&args.config) {
        Ok(c) => c,
        Err(e) => {
    ...
    use arc_project::utils::address_cache::AddressCache;
    AddressCache::init()?;
    info!("Address cache initialized from root address.txt");

    // Initialize Database
    let db_manager = core_logic::database::DatabaseManager::new("arc.db").await?;
