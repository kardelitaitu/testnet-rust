...
impl GasManager {
    pub const MAX_FEE_GWEI_DEFAULT: f64 = 30.0;
...
mod tests {
    ...
    fn test_gas_default_constants() {
        assert_eq!(GasManager::MAX_FEE_GWEI_DEFAULT, 30.0);
    ...
    fn test_get_max_fee_below_config_cap() {
        ...
        let base_fee = U256::from(20_000_000_000u128); // 20 gwei base
        let result = mgr.get_max_fee(base_fee);
        assert_eq!(
            result,
            U256::from(21_000_000_000u128),
            "20 gwei base + 1 gwei priority = 21 gwei"
        );
    }
    ...
    fn test_get_max_fee_zero_base() {
        ...
        let result = mgr.get_max_fee(base_fee);
        assert_eq!(result, U256::from(1_000_000_000u128));
    }
    ...
    fn test_get_max_fee_at_config_cap() {
        let provider = Arc::new(Provider::<Http>::try_from("http://localhost:9999").unwrap());
        let mgr = GasManager::new(provider, 0.0);
        let base_fee = U256::from(29_000_000_000u128); // 29 gwei
        let result = mgr.get_max_fee(base_fee);
        assert_eq!(result, U256::from(30_000_000_000u128));
    }
