...
impl Task<TaskContext> for RevertWithReasonTask {
    ...
    async fn run(&self, ctx: TaskContext) -> Result<TaskResult> {
        ...
        let deploy_data = crate::utils::strip_push0(
            &hex::decode(revert_bytecode.trim_start_matches("0x")).unwrap(),
        );
        ...
        let contract_address = match pending_deploy {
            ...
            Err(e) => {
                debug!(
                    "RevertWithReason deploy submit failed, resyncing nonce: {}",
                    e
                );
                let _ = nonce_manager.resync().await;
                return Ok(TaskResult {
        ...
        let call_data =
            revert_contract.encode("revertWithMessage", (String::from("test revert reason"),))?;
