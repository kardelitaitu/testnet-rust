...
impl Task<TaskContext> for MemoryExpansionTask {
    ...
    async fn run(&self, ctx: TaskContext) -> Result<TaskResult> {
        ...
        let deploy_data = crate::utils::strip_push0(
            &hex::decode(memory_bytecode.trim_start_matches("0x")).unwrap(),
        );
        ...
        let contract_address = match pending_deploy {
            ...
            Err(e) => {
                debug!(
                    "MemoryExpansion deploy submit failed, resyncing nonce: {}",
                    e
                );
                let _ = nonce_manager.resync().await;
                return Ok(TaskResult {
        ...
        match pending_process {
            ...
            Err(e) => {
                debug!(
                    "MemoryExpansion process submit failed, resyncing nonce: {}",
                    e
                );
                let _ = nonce_manager.resync().await;
                Ok(TaskResult {
