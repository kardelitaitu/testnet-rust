            ...
            Ok(pending) => {
                ...
                match pending.await {
                    Ok(Some(receipt)) if receipt.status == Some(U64::from(1)) => {
                        let deployed_addr = receipt.contract_address.context("No contract address in receipt")?;
                        Ok(TaskResult {
                            success: true,
                            message: format!("Transparent proxy deployed at {:?} (tx: {:?})", deployed_addr, tx_hash),
