...
#[async_trait]
impl Task<TaskContext> for Create2DeployTask {
    fn name(&self) -> &str {
        "24_create2Deploy"
    }
    ...
    async fn run(&self, ctx: TaskContext) -> Result<TaskResult> {
        ...
        match pending_tx {
            Ok(pending) => {
                ...
                match pending.await {
                    Ok(Some(receipt)) if receipt.status == Some(U64::from(1)) => {
                        // Try to extract the returned address from logs
                        ...
                        Ok(TaskResult {
                            success: true,
                            message: format!(
                                "CREATE2 deploy succeeded with salt {} ({}, tx: {})",
                    ...
                    Ok(Some(_)) => {
                        ...
                        Ok(TaskResult {
                            success: false,
                            message: format!("CREATE2 deploy reverted (tx: {})", tx_hash),
                            tx_hash: Some(tx_hash),
                        })
                    }
                    Ok(None) => {
                    ...
                    Err(e) => {
                        debug!("CREATE2 deploy receipt failed: {}", e);
                        let _ = nonce_manager.resync().await;
                        Ok(TaskResult {
                            success: false,
                            message: format!("CREATE2 deploy receipt error: {}", e),
                            tx_hash: Some(tx_hash),
                        })
                    }
                }
            ...
            Err(e) => {
                debug!("CREATE2 deploy submit failed, resyncing nonce: {}", e);
                let _ = nonce_manager.resync().await;
                Ok(TaskResult {
                    success: false,
