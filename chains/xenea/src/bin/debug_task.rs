...
async fn main() -> Result<()> {
    ...
    // Print Header first
    println!("--- XENEA Debugger ---");

    let args = Args::parse();
    // info!("Loading config from: {}", args.config);

    // 1. Load Config
    let cfg = match XeneaConfig::load(&args.config) {
        Ok(c) => c,
        Err(e) => {
    ...
    let proxies = if args.no_proxy {
        println!("Proxy loading disabled by --no-proxy");
        vec![]
    } else {
    ...
    xenea_project::utils::address_cache::AddressCache::init()?;
    println!("Address cache initialized from root address.txt");
    ...
    if args.all {
            ...
            .map(|i| {
                ...
                async move {
                    ...
                    let gas_manager = std::sync::Arc::new(
                        xenea_project::utils::gas::GasManager::new(provider_arc.clone()),
                    );
                        ...
                        Ok(decrypted) => {
                            ...
                            match key.parse::<LocalWallet>() {
                                Ok(w) => {
                                    ...
                                    let task = XeneaCheckBalanceTask;
        ...
        let tasks: Vec<Box<XeneaTask>> = vec![
            ...
            Box::new(XeneaEip7702ExploreTask),
            Box::new(VerifyCreate2Task),
            Box::new(DeployFactoryTask),
            Box::new(NonceRepairTask),
        ...
        let _task_duration_guard = {
            struct TaskDurationGuard(std::time::Instant);
            impl Drop for TaskDurationGuard {
                fn drop(&mut self) {
                    println!("⏱ Duration: {:.2?}", self.0.elapsed());
                }
            }

            TaskDurationGuard(std::time::Instant::now())
        };
