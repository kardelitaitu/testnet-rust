//! Decrypt Sepolia wallets, fetch EVM balances, and dump `address,balance` rows.
...
#[derive(Parser, Debug)]
#[command(
    name = "wallet-balance-dump",
    about = "Decrypt Sepolia wallets and dump EVM address/balance rows"
)]
struct Args {
    ...
    #[arg(short, long, default_value = "wallet-balances.txt")]
    output: String,
}

fn load_wallet_manager(config: &SepoliaConfig) -> Result<WalletManager> {
...
fn format_balance_4dp(balance: U256) -> Result<String> {
    let balance_eth: f64 = format_units(balance, 18)
        .context("Failed to format wei into ether")?
        .parse()
        .context("Failed to parse balance as floating point")?;
    Ok(format!("{balance_eth:.4}"))
}
...
async fn main() -> Result<()> {
    ...
    for idx in 0..total_wallets {
        ...
        let address = if !wallet.address().trim().is_empty() {
            ...
            eprintln!(
                "[wallet-balance-dump] skipping wallet {}: missing EVM address and private key",
                idx
            );
            continue;
        };
