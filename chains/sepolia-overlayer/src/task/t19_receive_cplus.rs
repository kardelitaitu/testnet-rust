...
impl SepoliaTask for ReceiveCplusTask {
    ...
    async fn run(&self, ctx: TaskContext) -> Result<TaskResult> {
        ...
        let tx = transfer_call.send().await.context("Failed to send C+ to proxy")?;
        ...
        let proxy_return = proxy_return_call.send().await.context("Proxy failed to send C+ back")?;
        ...
        let result_msg = if return_ok {
            ...
            format!(
                "Failed to receive USDC Plus - return receipt not confirmed (tx: {:?})",
                return_tx_hash,
            )
        };
