...
impl SepoliaTask for RedeemUsdcPlusTask {
    ...
    async fn run(&self, ctx: TaskContext) -> Result<TaskResult> {
        ...
        // --- 2. Calculate 5% of C+ balance, round down to nearest whole C+ ---
        ...
        let redeem_overlayer = whole_cplus * 1_000_000_000_000_000_000u128; // C+ amount (18 decimals)
        let collateral_amount = whole_cplus * 1_000_000u128; // USDC amount (6 decimals)

        if whole_cplus == 0 {
            return Ok(TaskResult {
                success: false,
                message: "2% of C+ balance rounds to 0, nothing to redeem".to_string(),
            });
        }
        ...
        Ok(TaskResult {
            ...
            message: format!(
                "Redeemed {} C+ → {} USDC (tx: {:?})",
                whole_cplus, whole_cplus, tx_hash
            ),
        })
    }
}
