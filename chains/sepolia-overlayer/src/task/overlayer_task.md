...
| t02 | mintUsdtPlus | Mint USDT+ (T+) using 1% of USDT balance |
| t03 | mintUsdcPlus | Mint USDC+ (C+) using 1% of USDC balance |
...
| t07 | stakeUsdcPlus | Stake USDC+ (C+) into C+ vault via `deposit(uint256,address)` (10% of C+, unlimited approve if needed) |
| t08 | unstakeTplus | Unstake sOverl... (shares) for T+ via `redeem(uint256,address,address)` (10% of sOverl..., min 0.01) |
| t09 | unstakeCplus | Unstake sOverl... (shares) for C+ via `redeem(uint256,address,address)` (10% of sOverl..., min 0.01) |
...
| t16 | bridgeBackTplus | Bridge T+ (USDT+) from Base Sepolia → Eth Sepolia via LayerZero OFT `send()` (20% of T+, ~0.0001 ETH fee) |
| t17 | bridgeBackCplus | Bridge C+ (USDC+) from Base Sepolia → Eth Sepolia via LayerZero OFT `send()` (20% of C+, ~0.0001 ETH fee) |
