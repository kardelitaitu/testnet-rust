...
/// sOverl... token (T+ staking vault shares) on Sepolia
const SOVL_TOKEN: &str = "0x079a4Bf1Cbd0E4ce15391340cB46efA6396aBc82";
/// sT+ farm pool on Sepolia (MasterChef-style deposit(uint256,uint256))
...
pub struct AddLiquiditySplusTask;

#[async_trait]
impl SepoliaTask for AddLiquiditySplusTask {
    fn name(&self) -> &str {
    ...
    async fn run(&self, ctx: TaskContext) -> Result<TaskResult> {
        ...
        Ok(TaskResult {
            ...
            message: if success {
                format!(
                    "Deposited {} sOverl into sT+ pool {} (tx: {:?})",
                    whole_sovl, POOL_ID, tx_hash
                )
            } else {
                format!(
                    "Failed to deposit sOverl into sT+ pool {} - receipt not confirmed (tx: {:?})",
                    POOL_ID, tx_hash
                )
            },
        })
...
mod tests {
    ...
    fn test_name_is_correct() {
        let task = AddLiquiditySplusTask;
        assert_eq!(task.name(), "27_addLiquiditySplus");
    }
