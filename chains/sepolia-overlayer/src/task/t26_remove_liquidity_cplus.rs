...
/// Liquidity pool contract for C+ on Sepolia (MasterChef-style deposit/withdraw)
...
pub struct RemoveLiquidityCplusTask;

#[async_trait]
impl SepoliaTask for RemoveLiquidityCplusTask {
    fn name(&self) -> &str {
    ...
    async fn run(&self, ctx: TaskContext) -> Result<TaskResult> {
        ...
        // --- 1. Check staked C+ balance in pool ---
        let staked_balance = get_staked_balance(provider, address, pool_addr).await?;

        // --- 2. Calculate 5% of staked balance, round to nearest whole C+ ---
        let withdraw_amount = calc_pct_rounded(staked_balance.as_u128(), 5, 100, 18);
        let dec18: u128 = 1_000_000_000_000_000_000;
        let whole_cplus = withdraw_amount / dec18;

        if whole_cplus == 0 {
            return Ok(TaskResult {
                success: false,
                message: "5% of staked C+ rounds to 0, nothing to withdraw".to_string(),
            });
        }
        ...
        Ok(TaskResult {
            success,
            message: if success {
                format!("Withdrew {} C+ from pool {} (tx: {:?})", whole_cplus, POOL_ID, tx_hash)
            } else {
                format!(
                    "Failed to withdraw C+ from pool {} - receipt not confirmed (tx: {:?})",
                    POOL_ID, tx_hash
                )
            },
        })
...
mod tests {
    ...
    fn test_name_is_correct() {
        let task = RemoveLiquidityCplusTask;
        assert_eq!(task.name(), "26_removeLiquidityCplus");
    }
