...
/// sT+ farm pool on Sepolia (MasterChef-style deposit/withdraw)
const SPLUS_POOL: &str = "0xc68fFFBfEC83a27Eae65DAe27BD6D6147cB3d765";
...
pub struct RemoveLiquiditySplusTask;

#[async_trait]
impl SepoliaTask for RemoveLiquiditySplusTask {
    fn name(&self) -> &str {
    ...
    async fn run(&self, ctx: TaskContext) -> Result<TaskResult> {
        ...
        let pool_addr: Address = SPLUS_POOL.parse()?;

        // --- 1. Check staked sOverl balance in sT+ pool ---
        ...
        if whole_sovl == 0 {
            return Ok(TaskResult {
                success: false,
                message: "5% of staked sOverl in sT+ pool rounds to 0, nothing to withdraw".to_string(),
            });
        }
        ...
        Ok(TaskResult {
            ...
            message: if success {
                format!(
                    "Withdrew {} sOverl from sT+ pool {} (tx: {:?})",
                    whole_sovl, POOL_ID, tx_hash
                )
            } else {
                format!(
                    "Failed to withdraw sOverl from sT+ pool {} - receipt not confirmed (tx: {:?})",
                    POOL_ID, tx_hash
                )
            },
        })
...
mod tests {
    ...
    fn test_name_is_correct() {
        let task = RemoveLiquiditySplusTask;
        assert_eq!(task.name(), "29_removeLiquiditySplus");
    }
