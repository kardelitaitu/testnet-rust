...
async fn main() -> Result<()> {
    ...
    let task_stream = futures::stream::iter(tasks).enumerate().map(|(id, task): (usize, Box<RobinhoodTask>)| {
        ...
        async move {
            ...
            let decrypted = wallet_manager.get_wallet(wallet_idx, wallet_password.as_deref()).await.unwrap();
            let wallet: LocalWallet = decrypted.evm_private_key.parse::<LocalWallet>().unwrap().with_chain_id(config.chain_id);
            
            let proxy_conf = if proxies.is_empty() { None } else { Some(&proxies[id % proxies.len()]) };
            ...
            let provider = Provider::new(Http::new_with_client(reqwest::Url::parse(&config.rpc_url).unwrap(), client));
    ...
    while let Some((id, name, duration, res)) = stream.next().await {
        match res {
            Ok(r) => println!("Task {:02}: {:<25} | {} | {:.2}s", id+1, name, if r.success { "✅" } else { "❌" }, duration.as_secs_f64()),
