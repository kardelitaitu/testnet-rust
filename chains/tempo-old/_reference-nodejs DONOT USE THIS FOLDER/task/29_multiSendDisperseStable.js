...
export async function multiSendDisperseStableForWallet(wallet, proxy, workerId = 1, walletIndex = 0, silent = false) {
    const startTime = Date.now();
    let actionName = '29_multiSendDisperseStable.js';
    ...
    try {
        ...
        if (!silent) console.log(`${COLORS.fg.yellow}🎲 Selected Stable: ${tokenName} (Balance: ${ethers.formatUnits(selected.balance, decimals)})${COLORS.reset}`);
        ...
        if (!silent) console.log(`${COLORS.fg.magenta}🚀 Batch Transferring Stable to ${recipients.length} addrs...${COLORS.reset}`);
