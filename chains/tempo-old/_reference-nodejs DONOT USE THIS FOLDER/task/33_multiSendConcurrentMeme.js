...
export async function multiSendConcurrentMemeForWallet(wallet, proxy, workerId = 1, walletIndex = 0, silent = false) {
    const startTime = Date.now();
    let actionName = '33_multiSendConcurrentMeme.js';
    ...
    try {
        // 1. Load Created Memes
        const dataPath = path.resolve(process.cwd(), 'data', 'created_memes.json');
        if (!fs.existsSync(dataPath)) throw new Error("created_memes.json not found");

        const data = JSON.parse(fs.readFileSync(dataPath, 'utf-8'));
        const myTokens = data[address] || [];
        if (myTokens.length === 0) {
            if (!silent) console.log(`${COLORS.dim}No meme tokens created yet. Skipping.${COLORS.reset}`);
            return { success: true, skipped: true, reason: 'no_created_tokens' };
        }
        ...
        const balanceChecks = await Promise.all(myTokens.map(async (t) => {
            ...
            } catch (e) { return { ...t, balance: 0n, decimals: 18 }; }
        }));

        const fundedTokens = balanceChecks.filter(t => t.balance > 0n);
        if (fundedTokens.length === 0) {
            if (!silent) console.log(`${COLORS.dim}No funded meme tokens found. Skipping.${COLORS.reset}`);
            return { success: true, skipped: true, reason: 'no_funded_tokens' };
        }
        ...
        const payments = selectedRecipients.map(to => {
            const amountDisplay = (Math.random() * 4.9 + 0.1); // 0.1 - 5.0 range
