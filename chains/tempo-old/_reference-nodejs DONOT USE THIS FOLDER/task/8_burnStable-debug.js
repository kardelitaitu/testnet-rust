import { burnRandomTokenForWallet } from './8_burnStable.js';
...
async function testBurnToken() {
    console.log(`  ${COLORS.fg.magenta}🐛  DEBUG MODE: Task 8 - Burn Token${COLORS.reset}\n`);
    ...
    // 2. Execute Task Once
    const result = await burnRandomTokenForWallet(wallet, proxy, 'DEBUG', 0, false);
...
testBurnToken().catch(console.error);
