...
const TRANSFER_AMOUNT_RANGE = { min: 10.0, max: 50.0 }; // Small amounts for Meme tokens

function getRandomInt(min, max) { return Math.floor(Math.random() * (max - min + 1)) + min; }

/**
 * Task 36: Native Batch Transactions (Meme)
 * Uses Tempo's native protocol-level batching for atomic execution via TempoSDKService.
 */
export async function batchSendTransactionMemeForWallet(wallet, proxy, workerId = 1, walletIndex = 0, silent = false) {
    const startTime = Date.now();
    let actionName = '36_batchSendTransactionMeme.js';
    ...
    try {
        ...
        if (availableTokens.length === 0) {
            throw new Error("No meme tokens found for this wallet in created_memes.json");
        }

        // 2. Select Funded Meme Token
        ...
        if (fundedTokens.length === 0) throw new Error("No funded meme tokens found.");
        ...
        if (!silent) console.log(`${COLORS.fg.cyan}[Worker ${workerId}] Batch Meme using ${tokenName} (Balance: ${ethers.formatUnits(tokenBalance, tokenDecimals)})${COLORS.reset}`);
        ...
        // 5. Send Atomic Batch Transaction using TempoSDKService
