...
fn test_security_utils_roundtrip_manual() {
    ...
    let decrypted = SecurityUtils::decrypt_components(&ciphertext_hex, &iv_hex, &salt_hex, &tag_hex, password)
...
fn test_security_utils_wrong_password() {
    ...
    let result = SecurityUtils::decrypt_components(&ciphertext_hex, &iv_hex, &salt_hex, &tag_hex, wrong_password);
