...
use core_logic::{
    AsyncDbConfig, ChainType, DatabaseManager, FallbackStrategy, MetricsCollector, ProxyRateLimiter, QueuedTaskResult,
    SpammerStats, WalletManager, WorkerRunner,
};
use std::sync::Arc;
...
impl Spammer for IntegrationSpammer {
    ...
    async fn start(&self, token: CancellationToken) -> Result<SpammerStats> {
        ...
        for i in 0..5 {
            ...
            let wallet = self
                .wallet_mgr
                .get_wallet_for_chain(wallet_idx, Some("pwd"), ChainType::Evm)
                .await;
            let address = wallet
                .map(|w| w.evm_address.clone())
            ...
            // 4. Log to DB
            self.db
                .queue_task_result(QueuedTaskResult {
